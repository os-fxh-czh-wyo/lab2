...
- Best-fit主要分为初始化、内存分配、内存释放等部分。
  
- 初始化阶段
  
  - 通过`best_fit_init()`函数为内存管理器创建基础数据结构
    
  - 通过`best_fit_init_memmap`对物理内存进行初始化，对每一页进行如下初始化操作：
...
- 利用`best_fit_alloc_pages`函数进行内存分配
  
  - 这里和`first-fit`的主要区别在于：我们需要在遍历空闲链表后，找到满足需求的最小空闲块，而不是直接使用第一个满足需求的空闲块。因此我们在找到一个满足需求的空闲块后，将会和已经记录的空闲块进行对比，如果它比记录的空闲块更小，我们就会更新记录值，然后继续寻找下一个空闲块。
  ...
  - 之后，和`first-fit`一样，如果我们使用的空闲块大小超出了需求，就需要分割超出的部分作为新空闲块，然后将剩余块插入原位置，保持链表的有序性。
    
- 利用`best_fit_free_pages`进行内存释放
  
  - 对于要释放的页块，首先将它们的属性设置为释放的页块数，然后将其标记为已分配状态，最后增加`nr_free`的值
...
- 目前Best-Fit还存在以下改进空间：
  ...
  - 遍历所有块的操作还可以进一步优化，如果可以将所有空闲块按照大小创建链表进行排序，就可以更高效地找到符合要求的最小空闲块。
    
  - 频繁进行空闲块分割可能会导致大量碎片的产生，可以定期对碎片进行整理。
    
  - 在搜索到可合并的块后立即进行合并，可能会使开销增大，可以尝试在定位所有可合并的空闲块之后，统一进行合并。
